...
class CustomComp extends Component {
        ...
        scriptUtil.getUserInfo(user => {
            scriptUtil.excuteScriptService({
                ...
                cb: (res) => {
                    ...
                    this.setState({ mainInfo: { ...this.state.mainInfo, ...temp } });
        ...
        return (
            ...
            >
                ...
                <div style={btnContainerStyle}>
                    <Button style={saveBtnStyle} onTouchStart={() => this.saveInfo()} disabled={this.state.submiting}> 切换 </Button>
                </div>
            </div>
        );
    }
    ...
    handleOpenAnalysisResult = () => {
        window.top.zhizhiDispatchAppEvent('reqOpenPublicAccount', {
            ...
            title: '',
            // 下一个页面的URL
            url: `http://60.167.69.246:38080/#/runtime-fullscreen/runtime-fullscreen/Page_0ff0edd66a394e27a77b1db7a51a42ce?date=${this.state.mainInfo.date}&time=${this.state.mainInfo.time}`
        })
    }
    //保存主子表
    saveInfo = () => {
        ...
        scriptUtil.excuteScriptService({
            ...
            params: { date: mainInfo.date, time: mainInfo.time },
            cb: (res) => {
                if (200 != res.code) {
                    return;
                }
                    ...
                    this.state.dataSections[0].data.map(objItem => {
                        objItem.list.map(proItem => {
                            if (proItem.value) {
                                _.set(struct, proItem.name, proItem.value);
                            }//proItem.name为xxx_CY
                            else {
