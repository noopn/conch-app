...
import { DatePicker, Icon, Upload, Checkbox, Input, message, Select, Tabs, Modal } from 'antd';
...
class CustomComp extends Component {
    ...
    componentWillMount() {
        ...
        scriptUtil.excuteScriptService({
            ...
            cb: (res) => {
                ...
                this.shipList = _.get(res, 'result.list', []);
            }
        });
        const arr = _.map(this.state, (value, key) => key);
        const obj = scriptUtil.getFormData(arr);

        obj.bgpic = JSON.parse(obj.bgpic ? obj.bgpic : '[]')
        this.setState({ ...obj });
        ...
        scriptUtil.excuteScriptService({
            objName: "BGGL",
            serviceName: "getBgBillCode",
            params: {},
            cb: (res) => {
                const Code = res.result[0].code;
                scriptUtil.setFormData({
                    Code
                });
            }
    ...
    boatChange = (value, key) => {
        this.setState({
            ...
            this.visible = true;
            this.shipList = this.filteredOptions.filter(x => {
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                <div style={topDivContainerStyle}>
                    <div style={topDescStyleZindex}>
                        <Input
                            disabled={this.disabledEdit}
                            style={commonStyle}
                            value={this.state.boatNo}
                            onChange={(e) => { this.boatChange(e.target.value, 'boatNo') }}
                        />
                        {
                            (this.visible && this.shipList.length > 0 && this.state.boatNo) ?
                                <ul
                                    style={searchTip}
                                    ref={ref => this.scrollDiv1 = ref}
                                    onTouchStart={this.handleTouchStart1}
                                    onTouchMove={this.handleTouchMove1}
                                ...
                                >
                                    {
                                        this.shipList.map((x) => {
                                            return (
                                                <li
                                                    style={singleItem}
                                                    onClick={(e) => { this.visible = false; this.instanceInputChange(x.shipNo, 'boatNo') }}
                                                    key={x.shipNo}
                                                >{x.shipNo}</li>
                                            )
                                        })
                    ...
                    <div style={topDescStyle}>
                        <Select
                            style={commonStyle}
                            onChange={(value) => this.yanmeiChange(value, 'coalType')}
                            value={this.state.coalType}
                        >
                            {
                                (this.state.yanmeiArr || []).map((item, index) => <Select.Option value={item.value}>{item.name}</Select.Option>)
                            }
                        </Select>
    ...
    handleTouchStart1 = (e) => {
        e.preventDefault();
        e.stopPropagation();
        this.startPosition1 = e.changedTouches[0].clientY;
    }
    ...
    handleTouchMove1 = (e) => {
        ...
        // 部分手机没有scrollBy
        // this.scrollDiv.scrollBy(0, this.startPosition - currentPosition);
        const scrollTop = this.scrollDiv1.scrollTop;
        this.scrollDiv1.scrollTop = scrollTop + this.startPosition1 - currentPosition;
        this.startPosition1 = currentPosition;
    }
