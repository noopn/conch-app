...
class CustomComp extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({
            mainInfo: {
                ...
                snName: serchQuery.cementType,
                snmType: serchQuery.cement_mill,
                createTime: serchQuery.createTime,
            }
        })
        //隐藏滚轮
        ...
        if (serchQuery.code) {
            // 修改时获取列表数据
            const inputs = '{"code":"' + serchQuery.code + '"}';
            ...
            scriptUtil.excuteScriptService({
                objName: "ZLGL",
                serviceName: "getFinalResult_CMSN",
                params: { code: serchQuery.code },
                cb: (res) => {
                    if (res.code != 200) {
                        this.showDialogue("数据获取异常");
                        return false;
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                <div style={topDivContainerStyle}>
                    <div style={topDescStyle}>{mainInfo.typeShowName}</div>
                                            ...
                                            {item.list.map((node, index) => (
                                                <div style={detailItemStyle}>
                                                    ...
                                                    <input
                                                        ...
                                                        type={node.name.substr(5) == 'BZ' || node.name.substr(5) == 'ZF' ? 'text' : 'number'}
                                                        placeholder=""
                                                        name="value"
                                                        value={node.value}
    ...
    handleOpenAnalysisResult = () => {
        scriptUtil.openPage(`#/runtime-fullscreen/runtime-fullscreen/Page_10259879e40f4c71a118b10238d7a282?code=${this.state.mainInfo.code}`, '_self');

        // window.top.zhizhiDispatchAppEvent('reqOpenPublicAccount', {
        //     // 下一个的最上面的标题
        //     title: '',
        //     // 下一个页面的URL
        //     url: `http://60.167.69.246:38080/#/runtime-fullscreen/runtime-fullscreen/Page_10259879e40f4c71a118b10238d7a282?code=${this.state.mainInfo.code} `
        // })
    }
    //保存主子表
    saveInfo = () => {
                ...
                this.state.dataSections[0].data.map(objItem => {
                    ...
                    objItem.list.map(proItem => {
                        if (proItem.value && parseFloat(proItem.value).toString() != "NaN") {
                            struct[proItem.name.split('_')[1]] = proItem.value;
                            // eval("struct." + proItem.name.split('_')[1] + "=" + proItem.value);
                        }//proItem.name为xxx_CY
                        else {
                            (proItem.value) ? struct[proItem.name.split('_')[1]] = "\"" + proItem.value + "\"" : struct[proItem.name.split('_')[1]] = null;
                        }
                    });
                    list.push(struct);
                });
