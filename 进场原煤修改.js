...
class CustomComp extends Component {
    ...
    componentDidMount() {
        ...
        if (serchQuery.code) {
            // 修改时获取列表数据
            const inputs = '{"code":"' + decodeURIComponent(serchQuery.code) + '"}';
            ...
            scriptUtil.excuteScriptService({
                objName: "ZLGL",
                serviceName: "getFinalResult_JCYM",
                params: { code: decodeURIComponent(serchQuery.code) },
                cb: (res) => {
                    if (res.code != 200) {
                        this.showDialogue("数据获取异常");
                        return false;
    ...
    get pCode() {
        if (this.state.mainInfo.place) {
            const arr = this.state.mainInfo.code.split('-');
            arr[2] = this.state.mainInfo.place;
            return arr.join('-')
        }
        return this.state.mainInfo.code
    }
    ...
    handleOpenAnalysisResult = () => {
        window.top.zhizhiDispatchAppEvent('reqOpenPublicAccount', {
            ...
            title: '',
            // 下一个页面的URL
            url: `http://60.167.69.246:38080/#/runtime-fullscreen/runtime-fullscreen/Page_301d584c0e9544c5a908607e564a8603?code=${this.pCode} `
        })
    }
    //保存主子表
    saveInfo = () => {
