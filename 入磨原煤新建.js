...
class CustomComp extends Component {
    ...
    componentDidMount() {
        ...
        scriptUtil.excuteScriptService({
            objName: "ZLGL",
            serviceName: "getYmNumber",
            params: {},
            cb: (res) => {
                const MFObjs = res.result.map(item => {
                    return {
                        ...item,
                    }
                })
                this.setState({ MFObjs: MFObjs });
            }
        });
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                <div style={topDivContainerStyle}>
                    <div style={topTitleStyle}>原煤样品</div>
    ...
    datePickChange = (date, dateString) => {
        if (date) {
            ...
            if (this.state.mainInfo.MFNum) {
                this.setDynamicCode(dateString, this.state.mainInfo.MFNum);
            }
        }
    ...
    selectChange = (value) => {
        const obj = this.state.MFObjs.find(section => section.name === value) || [];
        //如果目标对象与源对象有同名属性，或多个源对象有同名属性，则后面的属性会覆盖前面的属性。
        let data = Object.assign({}, this.state.mainInfo, { MFNum: obj.name, MFName: obj.showName });
        this.setState({ mainInfo: data });
    ...
    //获取自动编号
    setDynamicCode = (scDate, MFNum) => {
        scriptUtil.excuteScriptService({
            objName: "ZLGL",
            serviceName: "getDynamicCode_RMYM",
            params: { "scDate": scDate, "mfNum": MFNum, "type": billType },
            cb: (res) => {
    ...
    saveInfo = () => {
        ...
        const params = {
            ...
            "typeName": "RMYM", "typeShowName": "入磨原煤", "scName": mainInfo.MFNum, "scShowName": mainInfo.MFName,
            "scdate": mainInfo.scDate, "fxdate": mainInfo.fxDate, "code": mainInfo.code
        };
