...
class EditableCell extends React.Component {
    ...
    toggleEdit = (record, id) => {
        ...
        this.setState({ editing }, () => {
            if (editing) {
                // ht的mousedown事件会触发react的onblur事件,导致鼠标全选的时候回失去焦点， 需要阻止事件冒泡
                if (id) {
...
class CustomComp extends Component {
    ...
    // 过滤字段，处理服务返回的数据时，这些字段直接复制。不需要处理
    filterMapCopy = {
        mill: 'mill',
        // belt: 'belt',
        proDate: 'day',
        ...
        return columns.map(el => {
            ...
            if (!item.children) {
                item = {
                    ...
                    // 索引
                    dataIndex: item.key,
                    // 是否可编辑 ，可以不用 filterMap，直接在column数组中声明，见上面columns列数据
                    ...
                    render: (text, row, index) => {
                        ...
                        // 用于处理服务返回数据里有多行值合并列的情况，参照水泥生产班报的数据
                        if (item.key === 'mill') {
