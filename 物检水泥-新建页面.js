...
class CustomComp extends Component {
    ...
    initList = () => {
        _.map(['touchstart'], (event) => {
            ...
            this.selector1.addEventListener(event, (e) => {
                e.stopPropagation();
            });
        });
        ...
        scriptUtil.getUserInfo(user => {
            scriptUtil.excuteScriptService({
                ...
                cb: (res) => {
                    ...
                    this.setState({ mainInfo: { ...mainInfo, ...temp, billCode: selectDate } });
                }
            });
        });
                ...
                const chemicalList = res.result.map(item => {
                    return {
                        ...
                        list: item.list.map(item1 => ({
                            ...item1,
                            value: /CdTime|JsTime|ZdTime/.test(item1.name) ? moment(moment(new Date().getTime()).format('YYYY-MM-DD HH:mm'), 'YYYY-MM-DD HH:mm') : item1.value
                        }))
                    }
                })
                // 赋值
    ...
    saveInfo = () => {
        ...
        const params = {
            ...
            "typeName": "WJSN",
            "typeShowName": "物检水泥",
            "scName": mainInfo.snName,
            "scShowName": mainInfo.snName,
            "scdate": mainInfo.createTime,
            "fxdate": mainInfo.fxDate,
            "ysType": mainInfo.ysType,
            // "jcsl": 0,
            "code": mainInfo.code,
        ...
        scriptUtil.excuteScriptService({
            ...
            cb: (res) => {
                ...
                setTimeout(() => {
                    scriptUtil.openPage('http://60.167.69.246:38080/#/runtime-fullscreen/runtime-fullscreen/Page_354ff67e413c4f2d8168cfb9c2d4d0a3', '_self');
                }, 1500);
                // this.initData();
                // this.initList();
            }
